...
Object.keys(PALETTES).forEach((catId, ci)=>{
  ...
  for(let v=0; v<3; v++){
    ...
    WALLPAPERS.push({
      id, name:`${cat.label} ${["I","II","III"][v]}`, catId, catLabel:cat.label, group:cat.group,
      mood: variantMoods[v], colors: cat.colors||["gray"], stops: pal, gradType: type, gradAngle: angle,
      nativeDim: dim.id, bg, photo:false,
      views: Math.floor(seededRand(id*3.3)*4000)+120,
      downloads: Math.floor(seededRand(id*9.9)*1500)+20,
      hot: Math.floor(seededRand(id*1.7)*100)
    });
  }
});

// Real, properly-licensed photos layer on top of the gradient placeholders once
// data/photos.json exists (generated by scripts/import-wallpapers.mjs). Niches
// without real photos yet keep showing their gradient placeholder — nothing breaks.
async function mergeRealPhotos(){
  try{
    ...
    photos.forEach(p=>{
      const id = WID++;
      WALLPAPERS.push({
        id, name:p.name, catId:p.catId, catLabel:p.catLabel, group:p.group,
